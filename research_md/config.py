...
def register_project(project_path: str) -> dict:
    ...
    if not config:
        raise ValueError(
            f"No .research/research.json at {abs_path}. Call project_init there first."
        )
    if not config.get("id"):
        raise ValueError(
            f".research/research.json at {abs_path} has no 'id' field. Re-run project_init to generate one."
        )
...
def resolve_by_guid(guid: str) -> ResolvedProject | None:
    ...
    if _is_project(config) and config.get("id") == guid:
        ...
        return ResolvedProject(
            projectRoot=project_path, config=config, rootPath=root_path
        )
