...
.research/
  ...
  findings/
    0001-sqlite-zero-ops.md       <- HIGH evidence (2 sources + disconfirmation)
    0002-postgres-overkill.md     <- MODERATE evidence (vendor + secondary sources)
  candidates/
...
- Finding 0001 is **HIGH** — has 2 independent sources (sqlite.org + epicweb.dev) and a documented disconfirmation search ("SQLite production failures").
- Finding 0002 is **MODERATE** — has sources but no disconfirmation search, so it can't be upgraded to HIGH.
